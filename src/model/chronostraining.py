...
def main(
    ...
    training_data_paths = ["/opt/ml/input/data/train/kernelsynth-data.arrow"]
...
def model_fn(model_dir):
    ...
    model_path = os.path.join(model_dir, 'run-0/checkpoint-final/')
    model = ChronosPipeline.from_pretrained(model_path, local_files_only=True, use_safetensors=True, device_map=device, torch_dtype=torch.bfloat16)
