...
subprocess.run([sys.executable, "-m", "pip", "install", "gluonts"])
...
def parse_args():
    ...
    parser.add_argument(
        "--train_size", type=int, default=1_00, help="Training set rows"
    )
    parser.add_argument(
        "--validation_size", type=int, default=1_00, help="Validation set rows"
    )
    parser.add_argument(
        "--test_size", type=int, default=1_00, help="Test set rows"
    )
...
def generate_and_save_dataset(num_series, max_kernels, path):
    
    generated_dataset = Parallel(n_jobs=-1)(
        delayed(generate_time_series)(max_kernels=max_kernels)
...
if __name__ == "__main__":
    ...
    train_path = f'{BASE_DIR}/train/kernelsynth-data.arrow'
    validation_path = f'{BASE_DIR}/validation/kernelsynth-data.arrow'
    test_path = f'{BASE_DIR}/test/kernelsynth-data.arrow'

    # Call the dataset generation function with correct arguments
    generate_and_save_dataset(args['train_size'], args['max_kernels'], train_path)
    generate_and_save_dataset(args['validation_size'], args['max_kernels'], validation_path)
    generate_and_save_dataset(args['test_size'], args['max_kernels'], test_path)
    
    file_path = "/opt/ml/processing/train/kernelsynth-data.arrow"
    data_table = load_arrow_file(file_path)
    print(data_table)
